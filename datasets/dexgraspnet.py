...
class DexGraspNet(Dataset):
    ...
    def _pre_load_data(self, case_only: bool) -> None:
        ...
        # Sampling visualizers construct their inputs directly from scene_pcds;
        # they only need a non-empty map-style dataset container. Avoid loading
        # the full grasp training tensor for this case-only path.
        if case_only:
            scales_path = os.path.join(self.data_dir, 'scales.pkl')
            scales = pickle.load(open(scales_path, 'rb')) if os.path.isfile(scales_path) else {}
            self.dataset_info = {'num_per_object': {obj: 1 for obj in self.split}}
            for object_name in self.split:
                if object_name not in self.scene_pcds:
                    continue
                self.frames.append({
                    'robot_name': 'shadowhand',
                    'object_name': object_name,
                    'object_rot_mat': np.eye(3),
                    'qpos': torch.zeros(27).requires_grad_(True),
                    'scale': scales.get(object_name, 1.0),
                })
            return
        ...
        for mdata in grasp_dataset['metadata']:
            ...
            if object_name not in self.split:
                continue
