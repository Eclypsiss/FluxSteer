...
from engine.assets import find_object_assets
...
class IsaacGraspTestForce_shadowhand(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        # Resolve asset root path
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        self.asset_root = os.path.join(project_root, cfg["env"]["asset"]["assetRoot"])
...
#########六个方向都要满足
    def push_object(self):
        ...
        # 🔴 [Modified] Disable progress bar and debug prints
        ...
        # Replacement loop without tqdm and prints
        for i_direction in self.object_force_directions:
            i_achieve = self._push_object_with_direction(i_direction, None)
            achieve_6dir *= i_achieve
            achieve_6dir1 |= i_achieve
...
#         return achieve_6dir
    ...
    def _push_object_with_direction(self, i_direction, pbar):
        ...
        is_stable = self.is_obj_stable(object_pos_start, object_pos_terminal)
    ...
    def _load_obj_asset(self):
        ...
        if True:  # Skip tqdm indentation level
            cur = 0
            ...
            if self.object_name is None:
                ...
                object_urdf_path, object_mesh_path = find_object_assets(
                    self.cfg['dataset_dir'], self.object_name
                )
                
                # 🔧 修复URDF中的硬编码路径（DexGRAB等数据集）
                # URDF文件中的mesh路径可能是其他服务器的绝对路径，需要动态修正
                import tempfile
                import re
                
                # 将相对路径转为绝对路径
                if not os.path.isabs(object_urdf_path):
                    object_urdf_path = os.path.abspath(object_urdf_path)
                if not os.path.isabs(object_mesh_path):
                    object_mesh_path = os.path.abspath(object_mesh_path)
                
                # 读取URDF，检查是否需要修正mesh路径
                urdf_needs_fix = False
                with open(object_urdf_path, 'r') as f:
                    urdf_content = f.read()
                    # 检查URDF中的mesh路径是否存在
                    mesh_pattern = r'filename="([^"]+)"'
                    matches = re.findall(mesh_pattern, urdf_content)
                ...
                # 如果需要修正，创建临时URDF文件
                if urdf_needs_fix:
                    # 替换URDF中的mesh路径为实际的mesh路径
                    fixed_urdf_content = re.sub(
                        mesh_pattern,
                        f'filename="{object_mesh_path}"',
                        urdf_content
                    )
                    # 创建临时文件
                    temp_urdf = tempfile.NamedTemporaryFile(mode='w', suffix='.urdf', delete=False)
                    temp_urdf.write(fixed_urdf_content)
                    temp_urdf.close()
                    object_urdf_path = temp_urdf.name
                    # 记录临时文件路径，以便后续清理
                    if not hasattr(self, '_temp_urdf_files'):
                        self._temp_urdf_files = []
                    self._temp_urdf_files.append(object_urdf_path)
    ...
    def _load_obj(self, env_ptr, env_id):
        ...
        # ✅ 修复：UniDexGrasp的URDF中物体名字可能不是'object'，改为宽松检查
        obj_body_name = self.gym.get_actor_rigid_body_names(self.env_ptr_list[0], 1)[0]
        if obj_body_name != 'object':
            print(f"⚠️  Warning: Object body name is '{obj_body_name}', not 'object' (this is OK for UniDexGrasp)")
